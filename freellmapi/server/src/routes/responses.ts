...
responsesRouter.post('/responses', async (req: Request, res: Response) => {
  ...
  if (!quota.allowed) {
    res.status(429).json({ error: { message: `Monthly request quota reached (${quota.used}/${quota.limit} on the ${quota.plan} plan).`, type: 'quota_exceeded', code: 'monthly_quota_exceeded' } });
    return;
  }
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      if (stream) {
        ...
        await logRequest(orgId, route.platform, route.modelId, route.keyId, 'success', estimatedInputTokens, totalOutputTokens, Date.now() - start, null);
        return;
      } else {
        ...
        await logRequest(orgId, route.platform, route.modelId, route.keyId, 'success',
          promptTokens, completionTokens, Date.now() - start, null);
        return;
      }
    } catch (err: any) {
      const latency = Date.now() - start;
      await logRequest(orgId, route.platform, route.modelId, route.keyId, 'error', estimatedInputTokens, 0, latency, err.message);
