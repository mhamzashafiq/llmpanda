...
function Stat({ label, value, className }: { label: string; value: string | number; className?: string }) {
  return (
    <div className="rounded-2xl border bg-card px-4 py-3">
      ...
      <p className={`mt-1 font-display text-xl font-bold tabular-nums ${className ?? ''}`}>{value}</p>
    </div>
  )
}
              ...
              {byKey.length === 0 ? (
                ...
                <div className="max-h-[360px] overflow-y-auto -mx-4">
                  <Table>
                    ...
                    <TableBody>
                      {byKey.map((k: any, i: number) => (
                        <TableRow key={i}>
                          <TableCell className="pl-4 text-sm font-medium">
                            {k.label}
                            {k.keyless && <span className="ml-2 text-[10px] uppercase tracking-wide text-[#5fb13a]">free</span>}
                          </TableCell>
                          <TableCell className="text-xs text-muted-foreground">{k.platform}</TableCell>
                          <TableCell className="text-xs capitalize text-muted-foreground">{k.status}</TableCell>
                          ...
                          <TableCell className="text-right tabular-nums pr-4">{formatTokens(k.totalInputTokens + k.totalOutputTokens)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
