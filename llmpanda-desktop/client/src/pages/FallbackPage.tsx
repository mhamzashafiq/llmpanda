  ...
  return (
    <section className="rounded-lg border bg-card p-5">
      <div className="flex items-baseline justify-between mb-3">
        <h2 className="font-display text-sm font-bold uppercase tracking-wide">Monthly token budget</h2>
...
function Gauge({ label, used, limit }: { label: string; used: number; limit: number | null }) {
  const pct = limit ? Math.min(100, Math.round((used / limit) * 100)) : 0
  const color = !limit ? '#5fb13a' : pct >= 100 ? '#ff4d4f' : pct >= 80 ? '#f5a623' : '#5fb13a'
  return (
    <div>
...
function UsageGauges() {
  const { data = [] } = useQuery<UsageRow[]>({
    queryKey: ['fallback', 'usage'],
    queryFn: () => apiFetch('/api/fallback/usage'),
        ...
        {data.map(u => (
          <div key={`${u.platform}:${u.modelId}:${u.keyId}`} className="rounded-xl border border-border p-4">
            ...
            <div className="space-y-2">
              <Gauge label="RPM" used={u.rpm.used} limit={u.rpm.limit} />
              <Gauge label="RPD" used={u.rpd.used} limit={u.rpd.limit} />
              <Gauge label="TPM" used={u.tpm.used} limit={u.tpm.limit} />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-lg border bg-card p-5">
          <div className="flex items-baseline justify-between mb-3">
            <h2 className="font-display text-sm font-bold uppercase tracking-wide">Routing strategy</h2>
            {routing?.weights && (
              <span className="text-xs text-muted-foreground tabular-nums">
          ...
          <div className="inline-flex flex-wrap gap-1 rounded-full border p-1">
            {STRATEGIES.map(s => (
